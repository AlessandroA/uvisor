...
/* This struct contains all the static SRAM sections that uVisor allocates for a
 * secure box. It can be used to keep the sizes of the sections or their
 * pointers. */
typedef struct uvisor_bss_sections_t {
    /* Contains the size of the index, which is >= sizeof(UvisorBoxIndex). */
    uint32_t index;
    /* Contains the size of the user provided secure box context. */
    uint32_t context;
    /* Contains the size of the 3 RPC per-box data structures. */
    uint32_t rpc_outgoing_message_queue;
    uint32_t rpc_incoming_message_queue;
    uint32_t rpc_fn_group_queue;
    /* Contains the size of the secure box static heap. */
    uint32_t heap;
 ...
 * iterate over them to allocate the right space. */
...
typedef struct {
    ...
    /* The UvisorSramSizes struct is union-ed with a uint32_t array to allow for
     * loops to scan the sizes of all the SRAM sections and allocate the
     * necessary space for each of them. */
    union {
        uint32_t sizes[UVISOR_BSS_SECTIONS_COUNT];
        UvisorBssSections size_of;
    } const bss;

    /* Contains the size of the secure box static stack. */
    /* Note: This does not include guards. */
    /* Note: This is kept separately from the BSS sections as it's
     *       implementation specific where the stack sits wrt the BSS. */
