...
static void vmpu_sanity_checks_box_cfgtbl(uint8_t box_id, UvisorBoxConfig const * box_cfgtbl)
{
    ...
    if (box_cfgtbl->stack_size < UVISOR_MIN_STACK_SIZE) {
        HALT_ERROR(SANITY_CHECK_FAILED, "Box %i @0x%08X: Stack size (%uB) < UVISOR_MIN_STACK_SIZE (%uB).\r\n",
                   box_cfgtbl->stack_size, UVISOR_MIN_STACK_SIZE);
    }
...
static void vmpu_enumerate_boxes(void)
{
    ...
    for (box_cfgtbl = (const UvisorBoxConfig * *) __uvisor_config.cfgtbl_ptr_start;
        ...
        vmpu_sanity_checks_box_cfgtbl(box_id, *box_cfgtbl);
